...
export async function uploadNativeSavedMessagesFile(
    path: string,
    folderId: number | null,
    transferId?: string,
    conflictStrategy: UploadConflictStrategy = 'version'
): Promise<TelegramFile> {
    if (!isTauriRuntime() || !isSavedMessagesDefaultStorage()) {
        throw new Error('Native Saved Messages upload is only available in the desktop app.');
    }
    return await telegramUploadNativePath(path, folderId, transferId, conflictStrategy);
}
