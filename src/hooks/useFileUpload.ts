...
import { AppStore, invokeCommand, isSavedMessagesDefaultStorage, isTauriRuntime, listenEvent, openTauriFileDialog, uploadBrowserFile, uploadNativeSavedMessagesFile } from '../platform';
...
export function useFileUpload(activeFolderId: number | null, store: AppStore | null, targetLabel = 'Saved Messages') {
    ...
    const useDesktopFileDialog = isDesktopRuntime;
    ...
    const processItem = async (item: QueueItem) => {
        ...
        try {
            ...
            if (isDesktopRuntime && savedMessagesDefault) {
                await uploadNativeSavedMessagesFile(item.path, item.folderId, item.id, conflictStrategy);
            } else if (useDesktopFileDialog) {
            ...
            if (!cancelledRef.current.has(item.id)) {
                ...
                const shouldRetry = attemptNumber < MAX_AUTO_UPLOAD_ATTEMPTS && isRetryableUploadError(message);
...
function pickBrowserFiles(): Promise<File[]> {
    return new Promise((resolve) => {
        ...
        input.onchange = () => {
            ...
            window.setTimeout(() => input.remove(), 6 * 60 * 60 * 1000);
            resolve(files);
        };
...
function isRetryableUploadError(message: string): boolean {
    return !/file_parts_invalid|file_part_size_invalid|file_part_too_big|single-file upload limit|requested file could not be read/i.test(message);
}
