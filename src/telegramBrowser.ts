...
type TelegramInputFile = import('telegram/tl').Api.InputFile | import('telegram/tl').Api.InputFileBig;
type TelegramUploadPartRequest = import('telegram/tl').Api.upload.SaveFilePart | import('telegram/tl').Api.upload.SaveBigFilePart;

type NativeUploadedFile = {
    id: number;
    name: string;
    size: number;
...
const TELEGRAM_LARGE_FILE_THRESHOLD = 10 * 1024 * 1024;
const TELEGRAM_UPLOAD_KB = 1024;
const TELEGRAM_UPLOAD_MAX_PARTS = 4000;
const TELEGRAM_UPLOAD_MAX_PART_SIZE = 512 * TELEGRAM_UPLOAD_KB;
const TELEGRAM_SINGLE_FILE_MAX_SIZE = TELEGRAM_UPLOAD_MAX_PARTS * TELEGRAM_UPLOAD_MAX_PART_SIZE;
...
export async function telegramDeleteFile(messageId: number): Promise<boolean> {
    ...
    for (const chunk of record.chunks || []) {
        if (chunk.messageId === messageId) continue;
        const chunkKey = String(chunk.messageId);
        const chunkRecord = manifest.files[chunkKey];
        if (!chunkRecord) continue;
        manifest.files[chunkKey] = {
            ...chunkRecord,
            trashed: true,
            deletedAt,
            updatedAt: deletedAt,
...
export async function telegramRestoreFile(messageId: number): Promise<boolean> {
    ...
    for (const chunk of existing.chunks || []) {
        ...
        manifest.files[chunkKey] = {
            ...chunkRecord,
            folderId: manifest.files[key].folderId ?? null,
            trashed: false,
            deletedAt: undefined,
            missing: false,
            updatedAt: new Date().toISOString(),
        };
        manifest.fileFolders[chunkKey] = manifest.files[key].folderId ?? null;
    }
...
export async function telegramPermanentlyDeleteFile(messageId: number): Promise<boolean> {
    ...
    const client = await authorizedTelegramClient();
    const messageIds = record ? getRecordMessageIds(record) : [messageId];
    await client.deleteMessages('me', messageIds, { revoke: true });
    if (record) {
        rememberLocalFileLifecycle(record, 'deleted');
    }
    for (const id of messageIds) {
        delete manifest.fileFolders[String(id)];
        delete manifest.files[String(id)];
    }
    appendManifestEvent(manifest, 'file_deleted', { messageId, messageIds });
    await saveDriveManifest(manifest, 'debounced');
    return true;
}
...
export async function telegramUploadFile(
    ...
    if (file.size > TELEGRAM_SINGLE_FILE_MAX_SIZE) {
        return await uploadChunkedBrowserFile({
            client,
            manifest,
            file,
            folderId,
    ...
    const checksum = await safeSha256Blob(file);
...
export async function telegramUploadNativePath(
    path: string,
    folderId: number | null,
    transferId?: string,
    conflictStrategy: UploadConflictStrategy = 'version'
    ...
    const manifest = await getDriveManifest();
    const localName = getPathFilename(path);
    const strategy = normalizeUploadConflictStrategy(conflictStrategy);
    const conflicts = findActiveUploadConflicts(manifest, localName, folderId);
    const versionConflicts = strategy === 'version' ? conflicts : [];
    const replaceConflicts = strategy === 'replace' ? conflicts : [];
    const uploadName = strategy === 'keep_both'
        ? createUniqueFileName(manifest, localName, folderId)
        : localName;
    ...
    const versionGroup = versionConflicts[0]?.versionGroup || createVersionGroup(localName);
    const nextVersion = versionConflicts.length > 0
        ? Math.max(...versionConflicts.map((record) => record.version || 1)) + 1
        : 1;
    const updatedAt = new Date().toISOString();
    ...
    const uploadedParts = await invoke<NativeUploadedFile[]>('cmd_upload_file_chunks', {
        path,
        folderId: null,
        transferId,
    });
    if (uploadedParts.length > 1) {
        return await saveChunkedUploadRecord({
            manifest,
            uploadedParts,
            folderId,
            uploadName,
    ...
    const uploaded = uploadedParts[0];
    if (!uploaded) throw new Error('Native upload did not return file metadata');
    const messageId = Number(uploaded.id);
    ...
    for (const conflict of replaceConflicts) {
        ...
        appendManifestEvent(manifest, 'file_trashed', {
            ...createFileLifecyclePayload(trashed),
            replacedBy: messageId,
        });
    }

    manifest.fileFolders[String(messageId)] = folderId;
    manifest.files[String(messageId)] = {
        ...
        createdAt: updatedAt,
        updatedAt,
        mimeType: uploaded.mime_type || undefined,
        originalPath: path,
        integrityStatus: 'unknown',
        versionGroup: versionConflicts.length > 0 ? versionGroup : undefined,
        version: versionConflicts.length > 0 ? nextVersion : undefined,
    ...
    await saveDriveManifest(manifest, 'debounced');
    return recordToTelegramFile(manifest.files[String(messageId)]);
}

async function uploadChunkedBrowserFile(args: {
    ...
    versionConflicts: DriveFileRecord[];
    replaceConflicts: DriveFileRecord[];
    versionGroup: string;
    nextVersion: number;
    onProgress?: (percent: number) => void;
}): Promise<TelegramFile> {
    const partCount = Math.ceil(args.file.size / TELEGRAM_SINGLE_FILE_MAX_SIZE);
    const uploadedParts: NativeUploadedFile[] = [];
    let uploadedBytes = 0;

    for (let index = 0; index < partCount; index++) {
        const start = index * TELEGRAM_SINGLE_FILE_MAX_SIZE;
        const end = Math.min(start + TELEGRAM_SINGLE_FILE_MAX_SIZE, args.file.size);
        const partName = getChunkFileName(args.file.name, index, partCount);
        const partFile = new File([args.file.slice(start, end)], partName, {
            type: 'application/octet-stream',
            lastModified: args.file.lastModified || Date.now(),
        });
        const message = await sendTelegramFileWithRetry(args.client, 'me', {
            file: partFile,
            fileSize: partFile.size,
            forceDocument: true,
            workers: 1,
            progressCallback: (progress) => {
                const currentBytes = uploadedBytes + Math.round(partFile.size * progress);
                args.onProgress?.(Math.max(0, Math.min(99, Math.round((currentBytes / args.file.size) * 100))));
            },
        });
        uploadedParts.push({
            id: message.id,
    ...
    addBandwidth('up_bytes', args.file.size);
    args.onProgress?.(100);
    return await saveChunkedUploadRecord({
        ...
        versionConflicts: args.versionConflicts,
        replaceConflicts: args.replaceConflicts,
        versionGroup: args.versionGroup,
        nextVersion: args.nextVersion,
    });
}

async function saveChunkedUploadRecord(args: {
    manifest: DriveManifest;
    uploadedParts: NativeUploadedFile[];
    folderId: number | null;
    uploadName: string;
    originalPath: string;
    updatedAt: string;
    ...
    const parts = args.uploadedParts
        .map((part, index) => ({
            messageId: Number(part.id),
            index,
            size: Number(part.size) || 0,
            name: part.name || `${args.uploadName}.part${index + 1}`,
        }))
        .filter((part) => Number.isFinite(part.messageId));
    const firstPart = parts[0];
    if (!firstPart) throw new Error('Chunked upload did not return file metadata');
    const totalSize = parts.reduce((sum, part) => sum + part.size, 0);

    for (const conflict of args.replaceConflicts) {
        const trashed = moveFileRecordToTrash(args.manifest, conflict.messageId, args.updatedAt, conflict);
        rememberLocalFileLifecycle(trashed, 'trashed');
        appendManifestEvent(args.manifest, 'file_trashed', {
            ...createFileLifecyclePayload(trashed),
            replacedBy: firstPart.messageId,
        });
    }

    args.manifest.fileFolders[String(firstPart.messageId)] = args.folderId;
    args.manifest.files[String(firstPart.messageId)] = {
        messageId: firstPart.messageId,
        folderId: args.folderId,
        name: args.uploadName,
        size: totalSize,
        ...
        integrityStatus: 'unknown',
        chunks: parts,
        versionGroup: args.versionConflicts.length > 0 ? args.versionGroup : undefined,
        version: args.versionConflicts.length > 0 ? args.nextVersion : undefined,
        duplicateOf: args.versionConflicts[0]?.messageId,
    };

    for (const part of parts.slice(1)) {
        args.manifest.fileFolders[String(part.messageId)] = args.folderId;
        args.manifest.files[String(part.messageId)] = {
            ...
            originalPath: args.originalPath,
            integrityStatus: 'unknown',
            chunkOf: firstPart.messageId,
            chunkIndex: part.index,
        };
    }

    appendManifestEvent(args.manifest, 'file_uploaded', {
        messageId: firstPart.messageId,
        folderId: args.folderId,
        name: args.uploadName,
        size: totalSize,
        chunks: parts.length,
        chunkMessageIds: parts.map((part) => part.messageId),
        conflictStrategy: args.strategy,
        version: args.versionConflicts.length > 0 ? args.nextVersion : undefined,
    });
    if (args.versionConflicts.length > 0) {
        appendManifestEvent(args.manifest, 'duplicate_detected', {
            messageId: firstPart.messageId,
            duplicateOf: args.versionConflicts[0].messageId,
            count: args.versionConflicts.length,
        });
    }
    await saveDriveManifest(args.manifest, 'debounced');
    return recordToTelegramFile(args.manifest.files[String(firstPart.messageId)]);
}

function getChunkFileName(name: string, index: number, total: number): string {
    const width = Math.max(3, String(total).length);
    return `${name}.tdpart${String(index + 1).padStart(width, '0')}-of-${String(total).padStart(width, '0')}`;
}
...
export async function telegramGetCleanupSuggestions(): Promise<TelegramFile[]> {
    const manifest = await getDriveManifest();
    const records = Object.values(manifest.files).filter((record) => !record.chunkOf);
...
export async function telegramDownloadBlob(messageId: number): Promise<{ blob: Blob; name: string }> {
    ...
    const manifest = await getDriveManifest();
    const record = manifest.files[String(messageId)];
    if (record?.chunks && record.chunks.length > 1) {
        return await telegramDownloadChunkedBlob(client, record);
    }
...
async function telegramDownloadChunkedBlob(
    client: TelegramClientInstance,
    record: DriveFileRecord
): Promise<{ blob: Blob; name: string }> {
    const parts: BlobPart[] = [];
    for (const chunk of (record.chunks || []).slice().sort((a, b) => a.index - b.index)) {
        const message = await getTelegramMessage(chunk.messageId);
        const bytes = await downloadMessageBytes(client, message);
        parts.push(bytes);
        addBandwidth('down_bytes', bytes.byteLength);
    }
    return {
        blob: new Blob(parts, { type: record.mimeType || 'application/octet-stream' }),
        name: record.name,
    };
}
...
function normalizeLifecycleRecord(messageId: number, record: Partial<DriveFileRecord>): DriveFileRecord {
    ...
    return {
        ...
        integrityStatus: record.integrityStatus === 'valid' || record.integrityStatus === 'mismatch'
            ...
            : 'unknown',
        chunks: normalizeDriveFileChunks(record.chunks),
        chunkOf: record.chunkOf === undefined ? undefined : Number(record.chunkOf) || undefined,
        chunkIndex: record.chunkIndex === undefined ? undefined : Number(record.chunkIndex) || undefined,
    };
}

function normalizeDriveFileChunks(chunks: unknown): DriveFileChunk[] | undefined {
    if (!Array.isArray(chunks)) return undefined;
    const normalized = chunks
        .map((chunk, index) => {
            const item = chunk as Partial<DriveFileChunk>;
            ...
            return {
                ...
                name: String(item.name || `Telegram-file-${messageId}`),
            };
        })
        .filter((chunk): chunk is DriveFileChunk => Boolean(chunk))
...
function findActiveFileNameConflict(
    ...
    return Object.values(manifest.files)
        .filter((record) => !record.chunkOf && !record.trashed && !record.missing)
...
function getRecordMessageIds(record: DriveFileRecord): number[] {
    const ids = new Set<number>([record.messageId]);
    for (const chunk of record.chunks || []) {
        if (Number.isFinite(chunk.messageId)) ids.add(chunk.messageId);
    }
...
function getPathFilename(path: string): string {
    return path.replace(/\\/g, '/').split('/').pop() || 'unnamed';
}
...
async function safeSha256Blob(blob: Blob): Promise<string | undefined> {
    try {
        return await sha256Blob(blob);
    } catch {
        return undefined;
    }
}
...
async function sendBrowserFileWithRetry(
    ...
    for (let attempt = 0; attempt < TELEGRAM_UPLOAD_MAX_ATTEMPTS; attempt++) {
        try {
            await waitForTelegramUploadSlot();
            const fileHandle = await uploadBrowserFileInParts(client, file, {
...
async function uploadBrowserFileInParts(
    client: TelegramClientInstance,
    file: File,
    options: {
        workers: number;
        onProgress?: SendFileOptions['progressCallback'];
    }
): Promise<TelegramInputFile> {
    const { Api } = await import('telegram/tl');
    const { generateRandomBytes, readBigIntFromBuffer } = await import('telegram/Helpers');
    const fileId = readBigIntFromBuffer(generateRandomBytes(8), true, true);
    const isLarge = file.size > TELEGRAM_LARGE_FILE_THRESHOLD;
    const partSize = getTelegramUploadPartSize(file.size);
    const partCount = Math.max(1, Math.ceil(file.size / partSize));
    if (partCount > TELEGRAM_UPLOAD_MAX_PARTS) {
        throw new Error(`Telegram single-file upload limit is ${formatBytes(TELEGRAM_SINGLE_FILE_MAX_SIZE)}. Split this file and upload the parts separately.`);
    }
    const senderClient = client as TelegramClientInstance & {
        session: { dcId: number };
        getSender: (dcId: number) => Promise<{ send: (request: TelegramUploadPartRequest) => Promise<unknown>; isConnected: () => boolean }>;
    };
    let workers = Math.max(1, Math.min(options.workers || 1, partCount));
    let completed = 0;

    await senderClient.getSender(senderClient.session.dcId);
    options.onProgress?.(0);

    for (let firstPart = 0; firstPart < partCount; firstPart += workers) {
        const lastPart = Math.min(firstPart + workers, partCount);
        ...
        for (let part = firstPart; part < lastPart; part++) {
            uploads.push((async () => {
                ...
                await sendTelegramUploadPart(senderClient, request);
                completed += 1;
                if (options.onProgress) {
                    if (options.onProgress.isCanceled) throw new Error('USER_CANCELED');
                    options.onProgress(completed / partCount);
                }
            })());
        }

        await Promise.all(uploads);
        workers = Math.max(1, Math.min(workers, partCount - lastPart));
    }

    const name = file.name.split(/[\\/]/).pop() || file.name || 'unnamed';
    return isLarge
        ? new Api.InputFileBig({ id: fileId, parts: partCount, name })
        : new Api.InputFile({ id: fileId, parts: partCount, name, md5Checksum: '' });
}

async function sendTelegramUploadPart(
    client: TelegramClientInstance & {
        session: { dcId: number };
        getSender: (dcId: number) => Promise<{ send: (request: TelegramUploadPartRequest) => Promise<unknown>; isConnected: () => boolean }>;
    },
    request: TelegramUploadPartRequest
): Promise<void> {
    while (true) {
        ...
        try {
            ...
            if (floodSeconds !== null) {
                await sleep(Math.min((floodSeconds + 1) * 1000, 60_000));
                continue;
            }
            throw err;
        }
...
function getTelegramUploadPartSize(fileSize: number): number {
    if (fileSize < 100 * 1024 * 1024) return 128 * TELEGRAM_UPLOAD_KB;
    if (fileSize < 750 * 1024 * 1024) return 256 * TELEGRAM_UPLOAD_KB;
    return TELEGRAM_UPLOAD_MAX_PART_SIZE;
}
...
function isRetryableTelegramError(err: unknown): boolean {
    ...
    if (message.includes('file_parts_invalid') || message.includes('file_part_size_invalid') || message.includes('file_part_too_big')) {
        return false;
    }
...
function formatTelegramUploadError(err: unknown): string {
    ...
    if (/file_parts_invalid|file_part_size_invalid|file_part_too_big/i.test(message)) {
        return `Telegram rejected the file parts. This usually means the file is over Telegram's single-file upload limit (${formatBytes(TELEGRAM_SINGLE_FILE_MAX_SIZE)}). Split the file into smaller parts and upload those.`;
    }
